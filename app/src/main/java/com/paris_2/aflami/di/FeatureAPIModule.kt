...
object FeatureAPIModule {
    ...
    @Provides
    @Singleton
    fun provideMediaDetailsNavigator(): MediaDetailsNavigator = MediaDetailsNavigatorImpl(startGraph = MediaDetailsDestinations.MediaDetailsGraph1)

    @Provides
    @Singleton
    fun provideAuthenticationNavigator(): AuthenticationNavigator = AuthenticationNavigatorImpl(startGraph = AuthenticationDestinations.AuthenticationGraph1)
    ...
    @Provides
    @Singleton
    fun provideMediaDetailsFeatureAPI(@ApplicationContext context: Context): MediaDetailsFeatureAPI = MediaDetailsFeatureAPIImpl(context)

    @Provides
    @Singleton
    fun provideSearchFeatureAPI(@ApplicationContext context: Context): SearchFeatureAPI = SearchFeatureAPIImpl(context)

    @Provides
    @Singleton
    fun provideAuthenticationFeatureAPI(@ApplicationContext context: Context): AuthenticationFeatureAPI = AuthenticationFeatureAPIImpl(context)

    @Provides
    @Singleton
    fun provideAppNavigationAPI(@ApplicationContext context: Context): AppNavigationAPI = AppNavigationAPIImpl(context)
}
