...
interface SearchHistoryRepository {
    ...
    suspend fun addSearchHistory(searchHistoryModel: SearchHistoryModel)
    suspend fun clearSearchHistory(searchHistoryId: String)
    suspend fun clearAllSearchHistory()
}

class SearchHistoryRepositoryFakeImpl : SearchHistoryRepository {
    private val searchHistoryList = mutableListOf<SearchHistoryModel>(
        SearchHistoryModel(
            id = 1,
    ...
    override suspend fun addSearchHistory(searchHistoryModel: SearchHistoryModel) {
        searchHistoryList.add(searchHistoryModel)
    }

    override suspend fun clearSearchHistory(searchHistoryId: String) {
        searchHistoryList.removeIf { it.id.toString() == searchHistoryId }
    }
