...
import testUtils.fakeTvShowReviews
...
class GetTvShowReviewsUseCaseTest {
    ...
    fun `should return tv show review from repository`() = runTest {
        ...
        // when
        coEvery { tvShowRepository.getTvShowReview(tvShowId, page) } returns fakeTvShowReviews

        // Then
        val result = getTvShowReviewsUseCase(tvShowId,page)
        assertEquals(result, fakeTvShowReviews)
