...
class SearchViewModelTest {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `searchQuery should handle NoMediaForActorException without infinite loading`() = runTest {
        val query = "invalid_actor_name"

        // Mock the use case to throw NoMediaForActorException as it would for invalid actor names
        coEvery { searchByQueryUseCase(query, any()) } throws com.domain.search.exception.NoMediaForActorException()
        ...
        // Should not show loading state and should not have error message
        // because NoMediaForActorException should be handled as "no results" not an error
        assertThat(viewModel.screenState.value.isLoading).isFalse()
        assertThat(viewModel.screenState.value.errorMessage).isNull()
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `searchQuery should handle NoMediaForCountryException without infinite loading`() = runTest {
        val query = "invalid_country_name"

        // Mock the use case to throw NoMediaForCountryException as it would for invalid country names
        coEvery { searchByQueryUseCase(query, any()) } throws com.domain.search.exception.NoMediaForCountryException()
        ...
        // Should not show loading state and should not have error message
        // because NoMediaForCountryException should be handled as "no results" not an error
        assertThat(viewModel.screenState.value.isLoading).isFalse()
        assertThat(viewModel.screenState.value.errorMessage).isNull()
