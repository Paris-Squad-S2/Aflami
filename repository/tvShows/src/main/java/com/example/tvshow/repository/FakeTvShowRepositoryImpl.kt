...
class FakeTvShowRepositoryImpl : TvShowRepository {
    private val listOfTvShow = mutableListOf<TvShow>()
    private val favoriteTvShow = mutableListOf<TvShow>()
    ...
    override suspend fun getTvShowDetails(tvShowId: Int): TvShow {
        return fakeTvShows.find { it.id == tvShowId } ?: fakeTvShow
    }
    ...
    override suspend fun addTvShowToFavorite(tvShowId: Int) {
        val currentTv = fakeTvShows.find { it.id == tvShowId }
        if (currentTv != null) {
            favoriteTvShow.add(currentTv)
        }
    }
